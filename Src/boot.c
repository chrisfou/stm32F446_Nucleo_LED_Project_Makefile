...
const CMD_t C_CMDS[NB_CMD_MAX] = {{(uint8_t*)"LED", LED_CMD }};
      ...
      while ((G_pt_char = FIFO_PopChar(&G_Fifo_RX)) != NULL)
        {
          ...
          switch (*G_pt_char)
            ...
            case CARRIAGE_RETURN:
                  ...
                  for (uint8_t id_cmd=0; id_cmd < NB_CMD_MAX; id_cmd ++)
                    {   
                      if (strncmp((char*)C_CMDS[id_cmd].Pt_CmdName, 
                                  ...
                                  G_ARGS[0].len) == 0)
                        {
                          /* Un flag est levé pour memoriser le traitement 
                             de la commande. */
                          l_cmd_exe_flag = 1;

                          /* Execution de la commande */
                          l_cmd_result =
                            (*C_CMDS[id_cmd].Pt_FuncCmd)(G_nb_arg, G_ARGS);

                          /* Test le retour de la commande et affiche 
              ...
              /* Affichage par défaut du prompt sur reception 
                 d'un retour chariot */
              UART_SendStringOnIt(C_Msg_Prompt, 
                                  strlen((char*)C_Msg_Prompt));

              /* Le buffer de reception est vidé pour permettre 
                 la reception d'une nouvelle commande par l'utilisateur.*/
              G_Rx_Msg_Cmd_Id=0;
              break;
            ...
            default:
              ...
              /* Si le buffer de commande est plein alors emission 
                 d'un message d'erreur */
...
static void FIFO_PushChar (FIFO_t *pt_fifo, uint8_t thechar)
{
  ...
  FIFO_IncIdx(&(pt_fifo->idx_in));
  
  if (pt_fifo->idx_out == pt_fifo->idx_in)
    {
      FIFO_IncIdx(&(pt_fifo->idx_out));
    }
}
